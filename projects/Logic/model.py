...
class TimetableScheduler():
    ...
    def add_constraints(self):
        ...
        if self.optional_constraints:
            # optional constraint: the sessions of a course happens at most once a day
    ...
    def solve(self):
        ...
        if c == sat:
            ...
            if self.optional_constraints:
                print("Currently using the model with additional constraints. Consider initializing another scheduler without the optional constraints.")

    def print_schedule_df(self) -> pd.DataFrame:
    ...
    def draw_calendar(self, name: str, by: Literal['cds', 'prof', 'course', 'room'], id: int):
        ...
        with open(fname := f"./timetables/{name}_{by}_{id}.txt", 'w') as f:
            for (T, C, A) in schedule:
                ...
                # - name
                # - day
                if current_course != C or current_room != A or T - t_f > 1 or map_start[t_s] > map_start[T]:
