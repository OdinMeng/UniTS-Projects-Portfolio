...
# Usage - Creation of the SQL Database
**Step 1**: Create the database with empty tables. `sqlite3 ./databases/<db_name>.db < ./fill_data/create_empty.sql`

**Step 2**: Fill the database with data. The SQL script must be created beforehand; see the examples for some ideas on how to write said script. `sqlite3 ./database/<db_name>.db < ./fill_data/<sql_script>.sql`